...
import numpy as np
import pandas as pd
import joblib # For savings the model as a Binary File
...
def prepare_data(df):
  """it is use to separate the dependent variables and independent features

  Args:
      df (pd.DataFrame): it is a pandas DataFrame to  

  Returns:
      [tuple]: it returns tuple of ependent variables and independent variables
  """
...
def save_plot(df, file_name, model):
  ...
  def _create_base_plot(df):
    logging.info("creating the base model")
  ...
  logging.info("fsaving the plot at {plotpath}")
